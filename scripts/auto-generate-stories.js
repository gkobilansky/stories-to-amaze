 ...
 * Research products → Generate stories → Commit to GitHub
 */
...
async function autoGenerateStories(options = {}) {
  ...
  console.log('🤖 Starting automated story generation pipeline...\n');
  console.log(`📊 Config:`);
  ...
  console.log('🔍 STEP 1: Researching products...\n');
  ...
  if (readyProducts.length === 0) {
    console.error('❌ No products ready for stories. Try different category or more research.');
    return;
  }

  console.log(`\n✅ Found ${readyProducts.length} products ready for stories\n`);

  // Step 3: Generate stories
  const storiesToGenerate = Math.min(numStories, readyProducts.length);
  const results = [];

  console.log(`📝 STEP 2: Generating ${storiesToGenerate} stories...\n`);
  ...
  for (let i = 0; i < storiesToGenerate; i++) {
    ...
    try {
      ...
      console.log(`✅ Story generated: "${storyResult.title}"\n`);
